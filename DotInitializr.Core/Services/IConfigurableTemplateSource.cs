...
{
	public interface IConfigurableTemplateSource
	{
		/// <summary>
		/// Returns a file from a project template's source repository.<br/><br/>
		/// Provide a <paramref name="personalAccessToken"/> (PAT) for authentication or do not provide one for default (anonymous) authentication.
		/// </summary>
		...
		TemplateFile GetFile(string fileName, string sourceUrl, string sourceDirectory = null, string sourceBranch = null, string personalAccessToken = null);
		...
		IEnumerable<TemplateFile> GetFiles(string sourceUrl, string sourceDirectory = null, string sourceBranch = null, string personalAccessToken = null);
	}
}
